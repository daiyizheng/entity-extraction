...
from src.bert_models.Biaffine.utils import subword_label_alignment, label_list_to_entities, trans_label_MRC_EN
...
class BertBiaffineProcessor(object):
...
processors = {
    'bertbiaffine': BertBiaffineProcessor,
}
...
def convert_examples_to_features(examples:List[CutExample],
    ...
    """
    CutExample(id, text, entities, sub_id, cut_text, cut_entities, cut_number, 
               cut_len, labels, pre_labels, offsets_mapping)
    """
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        #"string" []     [()], ["O", "B-PER", "I-PER"]
        text, input_ids, offset_mapping, labels_lists = subword_label_alignment(text, entities, tokenizer)
        label_seqs = label_list_to_entities(labels_lists)
        ...
        if len(input_ids) > (max_seq_length - special_tokens_count):
            ...
            label_add_list = [list(label).copy() for label in label_seqs]
        ...
        assert len(label_ids_biaffine) == max_seq_length, "Error with slot labels length {} vs {}".format(len(label_ids_biaffine), max_seq_length)
        ...
        if ex_index < 10:
            ...
            print("label_seqs: %s", json.dumps(label_seqs, ensure_ascii=False))
        ...
        features.append(
            InputFeatures(
                ...
                slot_labels_ids=label_ids_biaffine
            )
        )
        example.offsets_mapping = offset_mapping
        example.labels = label_add_list
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    pad_token_label_id = args.label_map.get("O", 0)
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if mode == "train":
            examples = processor.get_examples(args.data_dir, mode, args.max_seq_length,
                                              tokenizer, args.data_type, special_tokens_count=2)
        elif mode == "dev":
            examples = processor.get_examples(args.data_dir, mode, args.max_seq_length,
                                              tokenizer, args.data_type, special_tokens_count=2)
        elif mode == "test":
    ...
    all_labels = torch.stack([f.slot_labels_ids for f in features])
