...
from src.bert_models.Biaffine.utils import transform_label_matrix2spans, Rm2entities, count_same_entities
...
class Trainer(object):
    def __init__(self, args, train_dataset=None, dev_dataset=None, test_dataset=None):
        ...
        self.config.simplify_biaffine = args.simplify_biaffine

        self.config.sampling_ratio = args.sampling_ratio
        self.config.use_focal = args.use_focal
        self.config.use_ffn = args.use_ffn
        self.config.use_lstm = args.use_lstm
        self.config.gradient_checkpointing = args.gradient_checkpointing
        self.config.output_dropout = args.output_dropout

        self.config.do_hard_negative_sampling = args.do_hard_negative_sampling
        self.config.hns_multiplier = args.hns_multiplier
    ...
    def train(self):
        ...
        if self.args.use_lstm:  # 是否需要10*
            linear1_param_optimizer = list(self.model.lstm.named_parameters())
            optimizer_grouped_parameters += [
        ...
        biaffine_param_optimizer = list(self.model.biaffine_classifiers.named_parameters())
        optimizer_grouped_parameters += [
            {'params': [p for n, p in biaffine_param_optimizer if not any(nd in n for nd in no_decay)],
             'weight_decay': self.args.weight_decay,
             'lr': self.args.biaffine_learning_rate
             },
    ...
    def evaluate(self):
        ...
        elif self.args.model_type.lower() == "bertbiaffine":
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            for i in range(labels.shape[0]):
                ...
                labeled_items = transform_label_matrix2spans(label, id2label_map=self.args.labels_list)#[(start, end, type)]
                labeled_items = sorted(labeled_items, key=lambda x: x[0], reverse=False)
                # print("labeled_items: ", labeled_items)
                pred_items = Rm2entities(
                    pred, is_flat_ner=self.args.is_flat_ner, id2label_map=self.args.labels_list
                )  # is_flat_ner=True #[(start, end, type)]
                pred_items = sorted(
        ...
        if self.args.eval_all_checkpoints and self.args.patience != 0:
            ...
            elif self.args.model_type == "bert_biaffine":
        ...
        for batch in tqdm(test_dataloader, desc="Prediction"):
            ...
            with torch.no_grad():
                ...
                for i in range(preds.shape[0]):
                    ...
                    slot_preds_list.append(pred_items)
