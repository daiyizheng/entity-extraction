...
def main():
    ...
    ## 模型输出目录
    args.output_dir = os.path.join(args.output_dir, args.model_type.lower()+"_"+ args.data_name + "_" + str(args.seed))
    ...
    args.tensorboardx_path = os.path.join(args.tensorboardx_path,
                                          args.model_type.lower() + "_" + args.data_name + "_" + str(
    ...
    labels_list = get_labels(os.path.join(args.data_dir, args.data_name, args.labels))
    ...
    logging.basicConfig(format="%(asctime)s - %(levelname)s - %(name)s - %(message)s",
                        datefmt="%m/%d/%Y %H:%M:%S",
                        level=logging.INFO,
                        filename=os.path.join(args.log_dir, args.model_type.lower() + "_" + args.data_name + "_" + str(args.seed)+".log"))
    ...
    if args.do_train:
        ...
        ##------------------- 加载训练 ------------------------------
        trainer = Trainer(
            args, tokenizer, config, model, train_dataset, dev_dataset, test_dataset
        )
    ...
    ## 加载旧的参数
    outputs_dir = os.path.join(args.output_dir, "checkpoint")
    load_args = torch.load(os.path.join(outputs_dir, 'training_args.bin'))
    load_args.do_eval = args.do_eval
    load_args.do_predict = args.do_predict
    args = load_args

    tokenizer = tokenizer_class.from_pretrained(outputs_dir, return_offsets_mapping=True,
                                                do_lower_case=args.do_lower_case,
                                                cache_dir=args.cache_dir if args.cache_dir else None)

    config = config_class.from_pretrained(outputs_dir,
                                          num_labels=args.num_labels,
                                          cache_dir=args.cache_dir if args.cache_dir else None,
                                          output_dropout=args.output_dropout,
                                          gradient_checkpointing=args.gradient_checkpointing)

    model = model_class.from_pretrained(outputs_dir,
                                        config=config,
                                        cache_dir=args.cache_dir if args.cache_dir else None,
                                        labels_list=args.labels_list,
                                        args=args)
    if args.do_eval:
        dev_dataset, _ = load_and_cache_examples(args, tokenizer, mode="dev")
        trainer = Trainer.load(args=args,
                               tokenizer=tokenizer,
