...
from src.bert_models.GlobalPointer.utils import ent2token_spans
...
class BertGlobalPointerProcessor(object):
...
processors = {
    'bertglobalpointer': BertGlobalPointerProcessor,
}
...
def convert_examples_to_features(examples:List[CutExample],
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        text, input_ids, offset_mapping, labels_seq = ent2token_spans(text, entities, tokenizer)
        tokens = [tokenizer.convert_ids_to_tokens(tk) for tk in input_ids]

        #设置闭区间-1, end位置-1，开区间
        labels_seq = [(label["start"], label["end"], label["type"]) for label in labels_seq]
        ...
        labels = np.zeros((len(label_map), max_seq_length, max_seq_length))
        for start, end, label in label_add_list:
            labels[label_map[label], start, end] = 1
        ...
        assert len(labels[0]) == max_seq_length, "Error with slot labels length {} vs {}".format(len(labels[0]), max_seq_length)
