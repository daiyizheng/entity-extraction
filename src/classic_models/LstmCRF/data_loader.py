...
from src.classic_models.LstmCRF.utils import ent2token_spans, spanlabel2seqlabel
...
def convert_examples_to_features(examples:List[CutExample],
    ...
    pad_token_id = tokenizer.word2id[pad_token]
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        labels = spanlabel2seqlabel(span_entity=labels_seq, length=len(offset_mapping))
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    pad_token_label_id = args.pad_token_label_id or args.label_map.get("PAD", -100)
