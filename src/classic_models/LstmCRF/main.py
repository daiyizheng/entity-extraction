...
from src.classic_models.LstmCRF import MODEL_CLASSES
from src.classic_models.LstmCRF.data_loader import load_and_cache_examples
from src.classic_models.LstmCRF.finetuning_argparse import get_argparse
from src.classic_models.LstmCRF.trainer import Trainer
from src.classic_models.LstmCRF.utils import override_defaults, get_vocab, get_embedding_matrix_and_vocab, seed_everything, get_labels
...
def main():
    ...
    args.output_dir = os.path.join(args.output_dir,
                                   args.model_type.lower() + "_" + args.task_name + "_" + str(args.seed))
    ...
    logging.basicConfig(format="%(asctime)s - %(levelname)s - %(name)s - %(message)s",
                        datefmt="%m/%d/%Y %H:%M:%S",
                        level=logging.INFO,
                        filename=os.path.join(args.log_dir, args.model_type.lower() + "_" + args.task_name + "_" + str(
                            args.seed) + ".log"))
    ...
    if not args.random_init_w2v:
        ...
        # vocab_list, vector_list = get_embedding_matrix_and_vocab(args.w2v_file, skip_first_line=True)
        vocab_list = get_vocab(args.vocab_dir)
        vector_list = []

    args.vocab_list = vocab_list
    args.vocab_size = len(vocab_list)
    args.vector_list = vector_list
    args.word2id = {word: idx for idx, word in enumerate(vocab_list)}

    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type.lower()]
    tokenizer = tokenizer_class.from_pretrained(unk_token=args.unk_token,
                                                pad_token=args.pad_token,
                                                word2id=args.word2id,
