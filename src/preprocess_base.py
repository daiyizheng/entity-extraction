...
import abc
import logging
from typing import List, Text, Tuple, Optional, Any

from src.schema import Entity, Relation, Event, Example
...
class PreProcessBase(abc.ABC):
    ...
    def cut_sentences(self,
                      text: Text,
                      entities: List[Entity])->List[Tuple[int, Text]]:
        ## 一阶截断
    ...
    def cut_sentences_v1(self,
                         sent_tupe: Tuple[int, Text])->List[Tuple[int, Text]]:
        """
        ...
        for index, char in enumerate(sent):
            if char == " " and index - 1 > 0 and (sent[index - 1] in self.punctuation_special_char):
                string += sent[index]
    ...
    def correct_ent_position(self,
                             cut_tuple: Tuple[int, Text],
                             events: List[Relation]) -> List[Relation]:
    ...
    def cut_text(self, example: Any)->List[Example]:
        """
